...
class googleimagesdownload:
        ...
        try:
            ...
            return (
                self._image_objects_from_pack(
                    self._extract_data_pack(respData), only_grid=only_grid
                ),
        ...
        for pky in prefix_keywords:  # 1.for every prefix keywords
            for sky in suffix_keywords:  # 2.for every suffix keywords
                ...
                while i < len(search_keyword):  # 3.for every main keyword
                    ...
                    if limit < 101:
                        images, tabs = self.download_page(
                            url, only_grid=arguments["only_grid"]
                        )
                    else:
    ...
    # Get number of images a query returns
    def query_images(self, arguments):
        paths_agg = {}
        # for input coming from other python files
        if __name__ != "__main__":
            # if the calling file contains config_file param
            if "config_file" in arguments:
                ...
                total_errors = 0
                for rec in records:
                    paths, errors = self.query_images_executor(rec)
                    for i in paths:
                        paths_agg[i] = paths[i]
                    if not arguments["silent_mode"]:
                        if arguments["print_paths"]:
            ...
            # if the calling file contains params directly
                ...
                paths, errors = self.query_images_executor(arguments)
                for i in paths:
                    paths_agg[i] = paths[i]
                if not arguments["silent_mode"]:
                    if arguments["print_paths"]:
    ...
    def query_images_executor(self, arguments):
        paths = {}
        errorCount = 0
        for arg in args_list:
            if arg not in arguments:
        ...
        for pky in prefix_keywords:  # 1.for every prefix keywords
            for sky in suffix_keywords:  # 2.for every suffix keywords
                ...
                while i < len(search_keyword):  # 3.for every main keyword
                    ...
                    if limit < 101:
                        images, _ = self.download_page(url, parse=True, only_grid=True)
                    else:
                        # not tested for extended
                        raise ValueError(
                            "Cannot get the number of images with a limit > 100"
                        )

                    path_key = pky + search_keyword[i] + sky
                    paths[path_key] = images
